...
export function configure_emscripten_startup(module: DotnetModule, exportedAPI: DotnetPublicAPI): void {
    ...
    const userPreInitAsync: (() => Promise<void>)[] = !module.preInitAsync ? [] : module.preInitAsync;
    const userPreRun: (() => void)[] = !module.preRun ? [] : typeof module.preRun === "function" ? [module.preRun] : module.preRun as any;
    const userPreRunAsync: (() => Promise<void>)[] = !module.preRunAsync ? [] : module.preRunAsync;
    const userpostRun: (() => void)[] = !module.postRun ? [] : typeof module.postRun === "function" ? [module.postRun] : module.postRun as any;
    const userpostRunAsync: (() => Promise<void>)[] = !module.postRunAsync ? [] : module.postRunAsync;
    // eslint-disable-next-line @typescript-eslint/no-empty-function
    const userOnRuntimeInitialized: () => void = module.onRuntimeInitialized ? module.onRuntimeInitialized : () => { };
    const userOnRuntimeInitializedAsync = module.onRuntimeInitializedAsync ? module.onRuntimeInitializedAsync : [];
    ...
    // execution order == [1] ==
    module.preInit = [() => preInit(isCustomStartup, userPreInit, userPreInitAsync)];
    // execution order == [2] ==
    module.preRun = [() => preRunAsync(userPreRun, userPreRunAsync)];
    // execution order == [4] ==
    module.onRuntimeInitialized = () => onRuntimeInitializedAsync(isCustomStartup, userOnRuntimeInitialized, userOnRuntimeInitializedAsync);
    // execution order == [5] ==
    module.postRun = [() => postRunAsync(userpostRun, userpostRunAsync)];
    // execution order == [6] ==
...
function preInit(isCustomStartup: boolean, userPreInit: (() => void)[], userPreInitAsync: (() => Promise<void>)[]) {
    ...
    (async () => {
        try {
            ...
            try {
                await Promise.all(userPreInitAsync.map(fn => fn()));
            }
            catch (err: any) {
                _print_error("MONO_WASM: user callback preInitAsync() failed", err);
                throw err;
            }
        } catch (err) {
            abort_startup(err, true);
...
async function preRunAsync(userPreRun: (() => void)[], userPreRunAsync: (() => Promise<void>)[]) {
    ...
    try {
        // all user Module.preRunAsync callbacks in parallel
        await Promise.all(userPreRunAsync.map(fn => fn()));
    } catch (err) {
        _print_error("MONO_WASM: user callback preRunAsync() failed", err);
        abort_startup(err, true);
        throw err;
    }
    // signal next stage
...
async function onRuntimeInitializedAsync(isCustomStartup: boolean, userOnRuntimeInitialized: () => void, userOnRuntimeInitializedAsync: (() => Promise<void>)[]) {
    // wait for previous stage
    ...
    try {
        ...
        try {
            await Promise.all(userOnRuntimeInitializedAsync.map(fn => fn()));
        }
        catch (err: any) {
...
async function postRunAsync(userpostRun: (() => void)[], userpostRunAsync: (() => Promise<void>)[]) {
    // wait for previous stage
    ...
    try {
        // all user Module.postRun callbacks in parallel
        await Promise.all(userpostRunAsync.map(fn => fn()));
    } catch (err) {
        _print_error("MONO_WASM: user callback posRunAsync() failed", err);
        abort_startup(err, true);
        throw err;
    }
    // signal next stage
...
async function _apply_configuration_from_args(): Promise<void> {
    try {
